...
int main(int argc, char *argv[])
{
  ...
  std::vector<int> smoothed(muonVec.size());
  int i = 0; // Transform raw data into smoothed data
  std::transform(muonVec.begin(), muonVec.end(), smoothed.begin(),
                 [&](auto val) { return get_3pt_avg<int>(muonVec, i++); });

  //PlotNormalizedMuons(muonVec);
  PlotNormalizedMuons(smoothed);
