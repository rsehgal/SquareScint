...
int main(int argc, char *argv[])
{
  ...
  TTree *treeML = new TTree("ftree", "A Tree for ML");
  std::vector<int> qVec;
  ...
  */
  ...
  for (unsigned int i = 0; i < vecOfVecOfHits.size(); i++) {
    ...
    */
    ...
    // if (IsSubset(hs, slab)) {
    if (IsSubset(hs, slab) || IsSubset(hs, topBar)) {
      // if (IsSubset(hs, slab2)) {
      // if (IsSubset(hs, bothSlabs)) {
      // if (IsSubset(hs, slab2)  && !IsSubset(hs, pmt4) && !IsSubset(hs, pmt5) && !IsSubset(hs, pmt6)&& !IsSubset(hs,
      // pmt7)){ if (IsSubset(hs, topBar)) { if (IsSubset(hs, darkRoomTopBar)) {
