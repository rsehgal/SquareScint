...
if set([0, 1]).issubset(unique_channels):
...
if set([2, 3]).issubset(unique_channels):
...
'''
# ✅ Plot all available time difference histograms on the same figure
plt.figure(figsize=(10, 5))
for col in df_grouped_energy.columns:
    if col.startswith("TimeDiff_"):
        plt.hist(df_grouped_energy[col], bins=100, histtype='step', alpha=0.7, label=col)
...
plt.hist(df_grouped_energy['Ratio_6_7'],histtype ='step', bins=100, alpha=0.7, label='CH_6 / CH_7', color='brown')
plt.xlabel("Energy Ratio")
plt.ylabel("Counts")
#plt.yscale('log')
plt.show()
...
plt.hist(df_grouped_energy['TimeDiff_6_7'],histtype ='step', bins=100, alpha=0.7, label='Time CH_6 - CH_7', color='black')
plt.xlabel("Time Difference (timestamp units)")
plt.ylabel("Counts")
#plt.yscale('log')
plt.show()
