...
#file = uproot.open("/home/barc/Harshita/DataF_Muon_Center_CrossBar_3600s_01Aug.root")
file = uproot.open("muon.root")
print(file.keys())
...
for row in data_sorted.itertuples(index=False):
    ...
    if group_start_time is None:
        ...
        if ts - group_start_time <= 20000:
            ...
            if len(current_group) == len(set(channels)) and len(current_group) == 8:
                #channels = [r.Channel for r in current_group]
...
plt.show()
'''
delt=[a-b for a,b in zip(T[0],T[2])]
p0 = [1.0, np.mean(delt),np.std(delt)]
count,bin_edges = np.histogram(delt,bins=50,density=True)
bin_centers = (bin_edges[:-1]+bin_edges[1:])/2

params,cov = curve_fit(gaussian,bin_centers,count,p0=p0)
A_fit,mu_fit,sigma_fit = params
print("mean : ",mu_fit)
print("sigma : ",sigma_fit)
plt.hist(delt, bins=500, histtype='step', edgecolor='red', align='left', label='Ch 3')
plt.show()
